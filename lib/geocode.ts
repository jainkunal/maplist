...
export async function geocodeViaNominatim(name: string, context?: string): Promise<{ lat: number; lng: number } | null> {
...
export async function geocodeViaGemini(name: string, context?: string): Promise<{ lat: number; lng: number } | null> {
  try {
    ...
    const locationText = lines.reverse().find((l: string) => l.includes(',')) ?? lines[0] ?? '';
...
export async function geocodePlace(name: string, context?: string): Promise<{ lat: number; lng: number }> {
